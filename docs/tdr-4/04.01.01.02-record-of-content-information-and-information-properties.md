...
APTrust uses tarred bags for submission and distribution (restoration). Metadata for preservation is primarily managed by the APTrust repository and [Bags](https://docs.aptrust.org/user-guide/depositing/) only need to contain a minimal amount of metadata. APTrust Bags need to establish fixity for all items preserved in its data directory through a secure cryptographic hash that confirms file integrity and mitigate against file tampering. Bagging services need to be able to confirm file checksums for items in the data directory match the hashes of the files from their original location to confirm proper hand-off and establish the integrity of the bag. The metadata generated while the SIP is in the APTrust work queue is described in [Work Items](https://docs.aptrust.org/user-guide/glossary/).

Submission packages are modeled in a local repository to manage metadata, help provide reporting and manage the preservation lifestyle over time. Submission Packages are read as part of the APTrust ingest process. Submission metadata is stored on the [Intellectual Object](https://docs.aptrust.org/user-guide/registry/objects/) that helps mediate access to the underlying digital files represented by the [Generic File](https://docs.aptrust.org/user-guide/registry/files/) objects. Technical metadata, data from audited events, and pointers to preservation files are stored in a Generic File object.  These digital objects are purposefully abstract to support the submission format agnostic packages to APTrust, while still allowing the depositor to specify a basic level of relationship by packaging files together or separately based on whatever relationship they see fit.
