...
description: >-
  The repository shall record and report to its administration all incidents
  of data corruption or loss, and steps shall be taken to repair/replace
  corrupt or…
number: "5.1.1.3.1"
...
- **Fixity Check** – This records the outcome of a fixity check on this file. APTrust perform fixity checks on items in Standard storage every 180 days, but does not perform fixity checks on items in Glacier-Only or Glacier Deep Archive storage. See [storage options](https://docs.aptrust.org/user-guide/depositing/#allowed-storage-option-values) for more information. Also note that APTrust checks the sha256 fixity only, even though we also know the md5 fixity value.

[Registry fixity checks](https://docs.aptrust.org/user-guide/registry/events/)
